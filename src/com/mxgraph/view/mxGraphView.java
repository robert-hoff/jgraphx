...
public class mxGraphView extends mxEventSource {
  ...
  public void updateEdgeState(mxCellState state, mxGeometry geo) {
    ...
    if ((graph.getModel().getTerminal(state.getCell(), true) != null && source == null)
      ...
      if (state.getCell() != getCurrentRoot() && (state.getAbsolutePointCount() < 2 || state.getAbsolutePoint(0) == null
   ...
   * @param source
   *          Boolean indicating if the next point for the source or target should
   *          be returned.
