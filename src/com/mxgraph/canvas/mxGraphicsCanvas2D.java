...
public class mxGraphicsCanvas2D implements mxICanvas2D {
  ...
  public void ellipse(double x, double y, double w, double h) {
    currentPath = new GeneralPath();
    currentPath.append(new Ellipse2D.Double((state.dx + x) * state.scale, (state.dy + y) * state.scale, w * state.scale,
        h * state.scale), false);
  }
  ...
  public void image(double x, double y, double w, double h, String src, boolean aspect, boolean flipH, boolean flipV) {
    if (src != null && w > 0 && h > 0) {
      ...
      if (img != null) {
        ...
        if (img != null) {
          drawImage(createImageGraphics(bounds.x, bounds.y, bounds.width, bounds.height, flipH, flipV), img, bounds.x,
              bounds.y);
        }
      }
    }
  ...
  protected String createHtmlDocument(String text, String align, String valign, int w, int h, boolean wrap,
    ...
    css.append("line-height:"
        + ((mxConstants.ABSOLUTE_LINE_HEIGHT) ? Math.round(state.fontSize * mxConstants.LINE_HEIGHT) + " " + HTML_UNIT
    ...
    if ((state.fontStyle & mxConstants.FONT_STRIKETHROUGH) == mxConstants.FONT_STRIKETHROUGH) {
   ...
   */
  protected void htmlText(double x, double y, double w, double h, String str, String align, String valign, boolean wrap,
    ...
    if (textRenderer != null && rendererPane != null) {
      ...
      rendererPane.paintComponent(state.g, textRenderer, rendererPane, (int) Math.round(x / HTML_SCALE),
          (int) Math.round(y / HTML_SCALE), (int) Math.round(w / HTML_SCALE), (int) Math.round(h / HTML_SCALE), true);
  ...
  public void plainText(double x, double y, double w, double h, String str, String align, String valign, boolean wrap,
    ...
    if (state.fontColor != null) {
      ...
      for (int i = 0; i < lines.length; i++) {
        ...
        if (!lines[i].isEmpty()) {
          boolean isUnderline = (state.fontStyle & mxConstants.FONT_UNDERLINE) == mxConstants.FONT_UNDERLINE;
