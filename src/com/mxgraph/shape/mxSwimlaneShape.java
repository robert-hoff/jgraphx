...
public class mxSwimlaneShape extends mxBasicShape {
  ...
  protected double getTitleSize(mxGraphics2DCanvas canvas, mxCellState state) {
    return Math.max(0, mxUtils.getFloat(state.getStyle(), mxConstants.STYLE_STARTSIZE, mxConstants.DEFAULT_STARTSIZE)
        * canvas.getScale());
  };
  ...
  protected mxGraphicsCanvas2D configureCanvas(mxGraphics2DCanvas canvas, mxCellState state, mxGraphicsCanvas2D c) {
    ...
    if (!mxConstants.NONE.equals(fill) && !mxConstants.NONE.equals(gradient)) {
      ...
      c.setGradient(fill, gradient, b.getX(), b.getY(), b.getWidth(), b.getHeight(),
          mxUtils.getString(state.getStyle(), mxConstants.STYLE_GRADIENT_DIRECTION, mxConstants.DIRECTION_NORTH), 1, 1);
    } else {
      c.setFillColor(fill);
    }
   ...
   */
  protected void paintRoundedSwimlane(mxGraphics2DCanvas canvas, mxCellState state, double start, double r, String fill,
      boolean swimlaneLine) {
    mxGraphicsCanvas2D c = configureCanvas(canvas, state, new mxGraphicsCanvas2D(canvas.getGraphics()));
    double w = state.getWidth();
    double h = state.getHeight();
