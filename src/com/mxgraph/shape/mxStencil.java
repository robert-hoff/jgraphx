...
public class mxStencil implements mxIShape {
    ...
    if (strokeColor != null || fillColor != null) {
      ...
      if (fillColor != null) {
        if (gradientColor != null && !gradientColor.equals("transparent")) {
          canvas.setGradient(fillColor, gradientColor, state.getX(), state.getY(), state.getWidth(), state.getHeight(),
              direction, 1, 1);
        } else {
   ...
   * Draws the shadow.
   */
  protected void drawShadow(mxGraphicsCanvas2D canvas, mxCellState state, double rotation, boolean flipH, boolean flipV,
      mxRectangle bounds, double alpha, boolean filled, mxRectangle aspect) {
    ...
    mxPoint offset = mxUtils.getRotatedPoint(new mxPoint(mxConstants.SHADOW_OFFSETX, mxConstants.SHADOW_OFFSETY), cos,
        sin);
  ...
  protected void drawElement(mxGraphicsCanvas2D canvas, mxCellState state, Element node, mxRectangle aspect) {
    ...
    if (name.equals("save")) {
      ...
      double[] curves = mxUtils.arcToCurves(this.lastMoveX, this.lastMoveY, r1, r2, angle, largeArcFlag, sweepFlag, x,
          y);
      ...
      canvas.text(x0 + getDouble(node, "x") * sx, y0 + getDouble(node, "y") * sy, 0, 0, str, node.getAttribute("align"),
          node.getAttribute("valign"), false, "", null, false, rotation, null);
