...
public class mxGraphGenerator {
  ...
  public void getBipartiteGraph(mxAnalysisGraph aGraph, int numVerticesGroup1, int numVerticesGroup2) {
    ...
    for (int j = 0; j < numVerticesGroup2; j++) {
      ...
      int edgeNum = aGraph.getOpposites(aGraph.getEdges(currVertex, null, true, true, false, true), currVertex, true,
          true).length;
   ...
   * @param yDim
   * @param weights
   *          Return a king graph of size <b>xDim</b> x <b>yDim</b> Note that the
   *          minimum size is 4x4
   */
    ...
    for (int i = 0; i < numBranches; i++) {
      for (int j = 0; j < numVerticesInBranch; j++) {
        ...
        if (isBranchSizeEven && j == middleIndex - 1) {
          ...
          if (j + 1 < middleIndex) {
            // before middle
            currRatio = (i - 1f / Math.pow(currSize, 0.25) + 0.00000000000015f * Math.pow(currSize, 4))
                / numVertexesInPerimeter;
          } else {
            // after middle
            currRatio = (i + 1f / Math.pow(currSize, 0.25) - 0.00000000000015f * Math.pow(currSize, 4))
                / numVertexesInPerimeter;
   ...
   * @param graph
   * @param weighted
   *          if true, the edges will be weighted, otherwise all will have default
   *          value (1.0)
   * @param minWeight
   ...
   *          tree of graph, which doesn't have to be but can be minimal (this is
   *          faster than minimal spanning tree, so if you need any spanning tree,
   *          use this one) Self loops and multiple edges are automatically
   *          removed! Also, unconnected graphs are made connected!
