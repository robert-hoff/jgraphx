...
public class mxPerimeter {
  ...
  public static mxPerimeterFunction TrianglePerimeter = new mxPerimeterFunction() {
    ...
    public mxPoint apply(mxRectangle bounds, mxCellState vertex, mxPoint next, boolean orthogonal) {
      ...
      boolean vertical = direction.equals(mxConstants.DIRECTION_NORTH) || direction.equals(mxConstants.DIRECTION_SOUTH);
      ...
      if (base) {
        ...
        if ((vertical && next.getX() <= x + w / 2) || (!vertical && next.getY() <= y + h / 2)) {
          result = mxUtils.intersection(next.getX(), next.getY(), cx, cy, start.getX(), start.getY(), corner.getX(),
              corner.getY());
        } else {
          result = mxUtils.intersection(next.getX(), next.getY(), cx, cy, corner.getX(), corner.getY(), end.getX(),
              end.getY());
        }
      }
