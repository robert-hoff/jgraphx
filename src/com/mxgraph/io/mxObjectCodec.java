...
public class mxObjectCodec {
   ...
   *          Value of the field.
   * @param isWrite
   *          Boolean indicating if the field is being encoded or decoded. isWrite
   *          is true if the field is being encoded, else it is being decoded.
  ...
  protected void encodeValue(mxCodec enc, Object obj, String fieldname, Object value, Node node) {
    if (value != null && !isExcluded(obj, fieldname, value, true)) {
      if (isReference(obj, fieldname, value, true)) {
        ...
        if (tmp == null) {
          log.log(Level.FINEST, "mxObjectCodec.encode: No ID for " + getName() + "." + fieldname + "=" + value);
          return; // exit
        }
  ...
  protected void writeComplexAttribute(mxCodec enc, Object obj, String attr, Object value, Node node) {
    ...
    if (child != null) {
      ...
      log.log(Level.FINEST, "mxObjectCodec.encode: No node for " + getName() + "." + attr + ": " + value);
    }
  }
  ...
  protected void setFieldValueWithAccessor(Object obj, Field field, Object value) {
    if (field != null) {
      try {
        ...
        log.log(Level.FINEST, "setFieldValue: " + e + " on " + obj.getClass().getSimpleName() + "." + field.getName()
            + " (" + field.getType().getSimpleName() + ") = " + value + " (" + value.getClass().getSimpleName() + ")",
            e);
      }
    }
  }
  ...
  protected void decodeAttribute(mxCodec dec, Node attr, Object obj) {
    ...
    if (!name.equalsIgnoreCase("as") && !name.equalsIgnoreCase("id")) {
      ...
      if (isReference(obj, fieldname, value, false)) {
        ...
        if (tmp == null) {
          log.log(Level.FINEST, "mxObjectCodec.decode: No object for " + getName() + "." + fieldname + "=" + value);
          return; // exit
        }
